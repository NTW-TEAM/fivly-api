...
export class AssemblyController {
  ...
  @Scopes(Scope.ASSEMBLIES_MANAGE)
  async createVoteSession(@Param('id') assemblyId: number, @Body() createVoteSessionDto: CreateVoteSessionDto){
    return this.assemblyService.createVoteSession(assemblyId, createVoteSessionDto);
  }

  @Patch(':id/vote-session/:voteSessionId')
  @Scopes(Scope.ASSEMBLIES_MANAGE)
  @HttpCode(204)
  async cancelVoteSession(@Param('id') assemblyId: number, @Param('voteSessionId') voteSessionId: number){
  ...
  @HttpCode(204)
  @Scopes(Scope.ASSEMBLIES_MANAGE)
  async deleteVoteSession(@Param('id') assemblyId: number, @Param('voteSessionId') voteSessionId: number){
    return this.assemblyService.deleteVoteSession(assemblyId, voteSessionId);
  }

  @Get(':id/vote-session/:voteSessionId')
  async getVoteSession(@Param('id') assemblyId: number, @Param('voteSessionId') voteSessionId: number){
  ...
  async vote(@Param('id') assemblyId: number, @Param('voteSessionId') voteSessionId: number, @Body() voteDto: VoteDto){
    return this.assemblyService.vote(assemblyId, voteSessionId, voteDto);
  }

  @Get(':id/vote-session/:voteSessionId/votes')
  async getVotes(@Param('id') assemblyId: number, @Param('voteSessionId') voteSessionId: number) {
