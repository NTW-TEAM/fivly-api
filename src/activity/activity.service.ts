...
export class ActivityService {
  ...
  async update(
    ...
    const activityType = await this.activityTypeRepository.findOneBy({
      name: updateActivityDTO.activityType,
    });
    if (!activityType) {
  ...
  async search(searchActivityDto: SearchActivityDTO): Promise<Activity[]> {
    ...
    if (searchActivityDto.activityType) {
      const activityType = await this.activityTypeRepository.findOneBy({
        name: searchActivityDto.activityType,
      });
      if (!activityType) {
        throw new BadRequestException('Activity type not found');
      }
    ...
    const where = Object.keys(searchActivityDto)
      .map((key) => {
        if (key === 'participant') {
  ...
  async create(createActivityDTO: CreateActivityDTO) {
    const activityType = await this.activityTypeRepository.findOneBy({
      name: createActivityDTO.activityType,
    });
