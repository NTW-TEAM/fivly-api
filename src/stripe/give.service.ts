...
export class GiveService {
  ...
  async createGiveSession(userId: number, crowdfundingId: number, amount: number) {
    const stripe = await this.getStripeClient();
    const association = await this.associationService.get();
    const user = await this.userRepository.findOne({where:{id:userId}});
    const crowdfunding = await this.crowdfundingRepository.findOne({where:{id:crowdfundingId}});
  ...
  async createCrowdfunding(createCrowdfundingDto: CreateCrowdfundingDto) {
    ...
    const user = await this.userRepository.findOne({where:{id:createCrowdfundingDto.creator}});
    if(!user){
  ...
  async getCrowdfundings(onlyActive?: boolean): Promise<Crowdfunding[]> {
    const queryBuilder = this.crowdfundingRepository.createQueryBuilder('crowdfunding')
  ...
  async endCrowdfunding(id: number) {
    ...
    const crowdfunding = await this.crowdfundingRepository.findOne({where:{id}});
    if (!crowdfunding) {
  ...
  async getGives(userId: number, crowdfundingId: number | undefined) {
    ...
    let queryBuilder = this.giveRepository.createQueryBuilder('give')
    ...
    if (crowdfundingId) {
      ...
      queryBuilder = queryBuilder.andWhere('crowdfunding.id = :crowdfundingId', { crowdfundingId });
    }

    // remove user from the query
    if(crowdfundingId){
      queryBuilder = queryBuilder.select(['give.id','give.amount', 'give.datetime']);
    } else {
      queryBuilder = queryBuilder.select(['give.id','give.amount', 'give.datetime', 'crowdfunding']);
    }
